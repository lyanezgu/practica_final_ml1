...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
rocc <- plot.roc(as.numeric(train_glm$nota_vino), as.numeric(train_glm$y_pred_probs2))
auc(rocc)
...
### 10.2.1. Optimización del valor de vecinos "K"
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
#predict(default_knn_mod, newdata = train_knn, type = "prob")
```  
