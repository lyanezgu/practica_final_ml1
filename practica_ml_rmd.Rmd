...
https://github.com/lyanezgu/practica_final_ml1
...
Para solucionar el problema de que una variable tenga más relevancia y sea más influyente por el hecho de tener más magnitud, se debe realizar una estandarización o escalado:
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
roc_glm <- plot.roc(as.numeric(train_glm$nota_vino), as.numeric(train_glm$y_pred_probs2), col = "blue")
auc(roc_glm)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
roc_knn <- plot.roc(as.numeric(train_knn$nota_vino), as.numeric(train_knn$y_pred_probs2), col = "green")
auc(roc_knn)
...
Creamos un modelo de árbol de decisión inicial básico y sin podar utilizando las 11 variables predictoras que tenemos a nuestra disposición y utiizando el método de "Gini" como medida de la impureza:
...
train_tree$y_pred_probs2 <- predict(caret.tree, newdata = train_tree, type = "prob")
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
roc_tree <- plot.roc(as.numeric(train_tree$nota_vino), as.numeric(train_tree$y_pred_probs2), col = "yellow")
auc(roc_tree)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
train_forest <- train %>% mutate(nota_vino = case_when(quality >= 6 ~1, TRUE ~ 0)) %>% mutate (quality= NULL)
train_forest
...
set.seed(4343)
rf <-randomForest(as.factor(nota_vino)~., data=train_forest, ntree=1000, do.trace=T, importance=T)
rf
...
caret.rf <- train(
  as.factor(nota_vino) ~ alcohol + volatile_acidity + Log_sulphates + Log_total_sulfur_dioxide, 
  data = train_forest, 
  method = "rf",
  ntree = 100,
  importance=TRUE,
...
train_forest$y_pred_probs2 <- predict(caret.rf, newdata = train_forest, type = "prob")
train_forest$y_pred_probs2 <- ifelse(train_forest$y_pred_probs2$`1` > 0.5, train_forest$y_pred_probs2$`1`, 1-train_forest$y_pred_probs2$`0`)

train_forest$y_pred2 <- ifelse(train_forest$y_pred_probs2 > 0.5, 1, 0)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
cm_train_forest <- confusionMatrix(as.factor(train_forest$y_pred2), as.factor(train_forest$nota_vino), positive='1')
cm_train_forest$table
#result
cm_train_forest$overall['Accuracy'] %>% round(2)
cm_train_forest$byClass['Recall'] %>% round(2)
cm_train_forest$byClass['Precision'] %>% round(2)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
roc_rf <- plot.roc(as.numeric(train_forest$nota_vino), as.numeric(train_forest$y_pred_probs2), col = "red")
auc(roc_rf)
...
train_en <- train %>% mutate(nota_vino = case_when(quality >= 6 ~1, TRUE ~ 0)) %>% mutate (quality= NULL)
train_en
...
train_en$y_pred_probs2 <- predict(caret.ada, newdata = train_en, type = "prob")
train_en$y_pred_probs2 <- ifelse(train_en$y_pred_probs2$`1` > 0.5, train_en$y_pred_probs2$`1`, 1-train_en$y_pred_probs2$`0`)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
cm_train_en <- confusionMatrix(as.factor(train_en$y_pred2), as.factor(train_en$nota_vino), positive='1')
cm_train_en$table
#result
cm_train_en$overall['Accuracy'] %>% round(2)
cm_train_en$byClass['Recall'] %>% round(2)
cm_train_en$byClass['Precision'] %>% round(2)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
roc_ada <- plot.roc(as.numeric(train_en$nota_vino), as.numeric(train_en$y_pred_probs2), col = "orange")
auc(roc_ada)
...
train_xgb <- train %>% mutate(nota_vino = case_when(quality >= 6 ~1, TRUE ~ 0)) %>% mutate (quality= NULL)
train_xgb
...
set.seed(2)
caret.xgb <- train(as.factor(nota_vino) ~ ., 
                   method = "xgbTree", 
                   data = train_xgb,
                   trControl = trainControl(method = "cv", number = 5, search = "grid", returnResamp = "final"))
caret.xgb
...
train_xgb$y_pred_probs2 <- predict(caret.xgb, newdata = train_xgb, type = "prob")
train_xgb$y_pred_probs2 <- ifelse(train_xgb$y_pred_probs2$`1` > 0.5, train_xgb$y_pred_probs2$`1`, 1-train_xgb$y_pred_probs2$`0`)

train_xgb$y_pred2 <- ifelse(train_xgb$y_pred_probs2 > 0.5, 1, 0)

#train_xgb$y_pred_probs2
#train_xgb$y_pred2
#train_xgb
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
cm_train_xgb <- confusionMatrix(as.factor(train_xgb$y_pred2), as.factor(train_xgb$nota_vino), positive='1')
cm_train_xgb$table
#result
cm_train_xgb$overall['Accuracy'] %>% round(2)
cm_train_xgb$byClass['Recall'] %>% round(2)
cm_train_xgb$byClass['Precision'] %>% round(2)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
roc_xgb <- plot.roc(as.numeric(train_xgb$nota_vino), as.numeric(train_xgb$y_pred_probs2), col = "purple")
auc(roc_xgb)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
train_svm <- train %>% mutate(nota_vino = case_when(quality >= 6 ~1, TRUE ~ 0)) %>% mutate (quality= NULL)
train_svm
...
#train_svm$y_pred_probs2 <- ifelse(train_svm$y_pred_probs2$`1` > 0.5, train_svm$y_pred_probs2$`1`, 1-train_svm$y_pred_probs2$`0`)

#train_svm$y_pred2 <- ifelse(train_svm$y_pred_probs2 > 0.5, 1, 0)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
cm_train_svm <- confusionMatrix(as.factor(train_svm$y_pred_probs2), as.factor(train_svm$nota_vino), positive='1')
cm_train_svm$table
#result
cm_train_svm$overall['Accuracy'] %>% round(2)
cm_train_svm$byClass['Recall'] %>% round(2)
cm_train_svm$byClass['Precision'] %>% round(2)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
roc_svm <- plot.roc(as.numeric(train_svm$nota_vino), as.numeric(train_svm$y_pred_probs2), col ="aquamarine3")
auc(roc_svm)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
cm_train_svm_radial <- confusionMatrix(as.factor(train_svm$y_pred_probs2), as.factor(train_svm$nota_vino), positive='1')
cm_train_svm_radial$table
#result
cm_train_svm_radial$overall['Accuracy'] %>% round(2)
cm_train_svm_radial$byClass['Recall'] %>% round(2)
cm_train_svm_radial$byClass['Precision'] %>% round(2)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
roc_svm_radial <- plot.roc(as.numeric(train_svm$nota_vino), as.numeric(train_svm$y_pred_probs2), col ="aquamarine4")
auc(roc_svm_radial)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
train_km <- train %>% mutate(nota_vino = case_when(quality >= 6 ~1, TRUE ~ 0)) %>% mutate (quality= NULL)
train_km
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
gam_mod2 = gam(quality ~ s(alcohol) + s(volatile_acidity) + s(Log_sulphates)+s(Log_chlorides),data = train, method = "REML")
...
#train_gam$y_pred_probs
#train_gam$y_pred

``` 

```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
cm_train_gam <- confusionMatrix(as.factor(train_gam$y_pred), as.factor(train_gam$nota_vino), positive='1')
cm_train_gam$table
#result
cm_train_gam$overall['Accuracy'] %>% round(2)
cm_train_gam$byClass['Recall'] %>% round(2)
cm_train_gam$byClass['Precision'] %>% round(2)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
roc_gam <- plot.roc(as.numeric(train_gam$nota_vino), as.numeric(train_gam$y_pred_probs), col = "pink")
auc(roc_gam)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
test_forest <- test %>% mutate(nota_vino = case_when(quality >= 6 ~1, TRUE ~ 0)) %>% mutate (quality= NULL)
test_forest
...
test_forest$y_pred_probs2 <- predict(caret.rf, newdata = test_forest, type = "prob")
test_forest$y_pred_probs2 <- ifelse(test_forest$y_pred_probs2$`1` > 0.5, test_forest$y_pred_probs2$`1`, 1-test_forest$y_pred_probs2$`0`)

test_forest$y_pred2 <- ifelse(test_forest$y_pred_probs2 > 0.5, 1, 0)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
cm_test_forest <- confusionMatrix(as.factor(test_forest$y_pred2), as.factor(test_forest$nota_vino), positive='1')
cm_test_forest$table
#result
cm_test_forest$overall['Accuracy'] %>% round(2)
cm_test_forest$byClass['Recall'] %>% round(2)
cm_test_forest$byClass['Precision'] %>% round(2)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
roc_rf_test <- plot.roc(as.numeric(test_forest$nota_vino), as.numeric(test_forest$y_pred_probs2), col = "red")
auc(roc_rf_test)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
test_xgb <- test %>% mutate(nota_vino = case_when(quality >= 6 ~1, TRUE ~ 0)) %>% mutate (quality= NULL)
test_xgb
...
test_xgb$y_pred_probs2 <- predict(caret.xgb, newdata = test_xgb, type = "prob")
test_xgb$y_pred_probs2 <- ifelse(test_xgb$y_pred_probs2$`1` > 0.5, test_xgb$y_pred_probs2$`1`, 1-test_xgb$y_pred_probs2$`0`)

test_xgb$y_pred2 <- ifelse(test_xgb$y_pred_probs2 > 0.5, 1, 0)

#test_xgb$y_pred_probs2
#test_xgb$y_pred2
#test_xgb
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
cm_test_xgb <- confusionMatrix(as.factor(test_xgb$y_pred2), as.factor(test_xgb$nota_vino), positive='1')
cm_test_xgb$table
#result
cm_test_xgb$overall['Accuracy'] %>% round(2)
cm_test_xgb$byClass['Recall'] %>% round(2)
cm_test_xgb$byClass['Precision'] %>% round(2)
...
```{r, include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
roc_xgb_test <- plot.roc(as.numeric(test_xgb$nota_vino), as.numeric(test_xgb$y_pred_probs2), col = "purple")
auc(roc_xgb_test)
